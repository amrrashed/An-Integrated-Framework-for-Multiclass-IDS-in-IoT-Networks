import pandas as pd
import numpy as np
from sklearn.preprocessing import MinMaxScaler, LabelEncoder
from sklearn.model_selection import train_test_split
from sklearn.neighbors import KNeighborsClassifier
from sklearn.metrics import accuracy_score
from tensorflow.keras.models import load_model
from bayes_opt import BayesianOptimization

# Load your dataset
df = pd.read_csv('D:/new researches/Security/dataset/processed win dataset/windows10_dataset.csv')

# Extract features and labels
X = df.drop(columns=['label', 'type'])
y = df['type']

# Apply label encoding to y
label_encoder = LabelEncoder()
y_encoded = label_encoder.fit_transform(y)

# Check for string values in X and encode them if they exist
for column in X.columns:
    if X[column].dtype == 'object':
        le = LabelEncoder()
        X[column] = le.fit_transform(X[column])
        print(f"Encoded column: {column}")

# Split into train test sets
X_train, X_test, y_train, y_test = train_test_split(X, y_encoded, test_size=0.2, random_state=1)



# Define the function to optimize
def knn_cv(n_neighbors, leaf_size, p, weights, algorithm):
    # Convert parameters to appropriate types
    n_neighbors = int(n_neighbors)
    leaf_size = int(leaf_size)
    p = int(p)
    weights = 'uniform' if weights < 0.5 else 'distance'
    algorithm = 'auto' if algorithm < 0.5 else 'ball_tree' if algorithm < 1.5 else 'kd_tree' if algorithm < 2.5 else 'brute'
    
    # Define the KNN model with given hyperparameters
    knn_model = KNeighborsClassifier(n_neighbors=n_neighbors, leaf_size=leaf_size, p=p, weights=weights, algorithm=algorithm)
    
    # Fit the model on the training set
    knn_model.fit(X_train, y_train)
    
    # Make predictions on the test set
    yhat = knn_model.predict(X_test)
    
    # Calculate classification accuracy
    acc = accuracy_score(y_test, yhat)
    
    return acc

# Define the parameter space
pbounds = {
    'n_neighbors': (1, 10),
    'leaf_size': (10, 50),
    'p': (1, 2),
    'weights': (0, 1),  # Uniform vs Distance
    'algorithm': (0, 3)  # Auto, Ball Tree, KD Tree, Brute
}

# Initialize Bayesian Optimization
optimizer = BayesianOptimization(
    f=knn_cv,
    pbounds=pbounds,
    random_state=1
)

# Maximize the objective function (accuracy)
optimizer.maximize(init_points=10, n_iter=50)

# Get the best parameters
best_params = optimizer.max['params']
best_n_neighbors = int(best_params['n_neighbors'])
best_leaf_size = int(best_params['leaf_size'])
best_p = int(best_params['p'])
best_weights = 'uniform' if best_params['weights'] < 0.5 else 'distance'
best_algorithm = 'auto' if best_params['algorithm'] < 0.5 else 'ball_tree' if best_params['algorithm'] < 1.5 else 'kd_tree' if best_params['algorithm'] < 2.5 else 'brute'

# Define the best KNN model with optimal hyperparameters
best_knn_model = KNeighborsClassifier(
    n_neighbors=best_n_neighbors, 
    leaf_size=best_leaf_size, 
    p=best_p, 
    weights=best_weights, 
    algorithm=best_algorithm
)

# Fit the model on the training set
best_knn_model.fit(X_train, y_train)

# Make predictions on the test set
best_yhat = best_knn_model.predict(X_test)

# Calculate classification accuracy
best_acc = accuracy_score(y_test, best_yhat)
print("Best Accuracy:", best_acc)
print(best_knn_model)